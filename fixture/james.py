...
class JamesHelper:
    ...
    def ensure_user_exists(self, username, password):  # логин пароль пользователя
    ...
    class Session:
        ...
        def __init__(self, host, port, username, password):  # логин пароль почтового сервера из target.json
        ...
        def is_users_registred(self, username):
            self.write("verify %s\n" % username)
            res = self.telnet.expect([b"exists", b"does not exist"])  # b - используются байтовые строки
            return res[0] == 0
